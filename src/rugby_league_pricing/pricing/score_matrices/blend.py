...
from .negative_binomial import build_negative_binomial_score_matrix
from ..score_matrices_legacy import (
...
def blend_score_matrices(
    ...
    *,
    ...
    if not 0 <= historical_weight <= 1:
        raise ValueError(
            "historical_weight must be between 0 and 1."
        )
    ...
    if not np.array_equal(
        ...
        raise ValueError(
            "Both matrices must use the same score grid."
        )

    probabilities = (
        (1 - historical_weight)
        * theoretical_matrix.probabilities
        + historical_weight
        * historical_matrix.probabilities
    )
...
def build_blended_score_matrix(
    ...
    *,
    ...
    """
    Build the production Negative-Binomial + tilted-historical matrix.
    """

    theoretical_matrix = (
        build_negative_binomial_score_matrix(
    ...
    return blend_score_matrices(
        theoretical_matrix=theoretical_matrix,
